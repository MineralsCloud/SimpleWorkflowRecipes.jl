...
[SimpleWorkflowRecipes](https://github.com/MineralsCloud/SimpleWorkflowRecipes.jl).
...
2. Run the following commands and wait for them to finish:
   ...
   julia> Pkg.add("SimpleWorkflowRecipes")
   ```

3. Run

   ```julia-repl
   julia> using SimpleWorkflowRecipes
   ```
...
version of SimpleWorkflowRecipes, type

```@repl
using Pkg
Pkg.update()
pkg"add https://github.com/MineralsCloud/SimpleWorkflowRecipes.jl"
```

in the second step above.

## Update SimpleWorkflowRecipes
...
our [GitHub repository](https://github.com/MineralsCloud/SimpleWorkflowRecipes.jl)
for new releases.
Once we release a new version, you can update SimpleWorkflowRecipes by typing

```@repl
using Pkg
Pkg.update("SimpleWorkflowRecipes")
Pkg.gc()
```

in the Julia REPL.

## Uninstall and reinstall SimpleWorkflowRecipes
...
1. To uninstall, in a Julia session, run
   ...
   julia> Pkg.rm("SimpleWorkflowRecipes")

   julia> Pkg.gc()
   ```

2. Press `ctrl+d` to quit the current session. Start a new Julia session and
   reinstall SimpleWorkflowRecipes.
